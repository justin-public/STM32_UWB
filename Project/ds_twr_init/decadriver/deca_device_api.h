 ...
 * 
 * 이 함수는 DW1000 트랜시버와의 통신을 시작하고, DEV_ID 레지스터(주소 0x00)를 읽어 
 * 이 소프트웨어에서 지원하는 IC인지 확인합니다(예: DW1000의 32비트 장치 ID 값은 0xDECA0130임). 
 * 그런 다음, 필요한 초기 1회 장치 구성을 수행하고, 이 저수준 드라이버에 속하는 정적 데이터 
 * 항목들을 초기화합니다.
 * 이 함수는 또한 OTP 메모리에서 LDO 튜닝 값과 크리스탈 트림 값을 읽어 적용합니다.
 * NOTES:
 ...
 * 
 * 1. 이 함수는 dwt_configuresleep을 실행하기 전에 실행되어야 하며, SPI 주파수는 3MHz 미만이어야 합니다.
 * 2. 이 함수는 또한 OTP 메모리에서 LDO 튜닝 값과 크리스탈 트림 값을 읽어 적용합니다.
 * input parameters
